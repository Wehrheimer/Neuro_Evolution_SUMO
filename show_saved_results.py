# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 12:37:14 2019
...
if __name__ == "__main__":
    path=r'H:\MT\Python\NEAT und SUMO\saved models'
    allfiles=os.listdir(path)
    neat_genomes=[]
    x=0
    
    for files in allfiles:
        if r'best_genome_neat' in allfiles[x]:
            fullpath=os.path.join(path, allfiles[x])
            neat_genomes.append(fullpath)
        x+=1
        
    neat_genomes.sort()
    
    with open(neat_genomes[-1], 'rb') as pickle_file:
...
#                   fitness_function(list(iteritems(self.population)), self.config, episode)
...
#    for genome_id, genome in genomes:
    ...
    #             net = neat.nn.FeedForwardNetwork.create(genome, config)
             ...
             SUMO.a_set[SUMO.step] = 20*SUMO.a_set[SUMO.step]-10
    ...
    #    print(v_episode)
        ...
        """Calculate Reward ================================================================================"""
        fitness = fitness = len(v_episode)*np.sqrt(np.sum(np.square(v_episode)))-np.sum(0.5*np.sign(features.v_allowed-np.asarray(v_episode))+1)-np.mean(np.square(a_episode))-1/sum(np.square(np.asarray(v_episode)*3.6-np.asarray(distance_episode)))-100000*int(SUMO.Collision)
