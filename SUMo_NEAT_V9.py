...
if __name__ == "__main__":
    ...
    number_episodes = 4000  # number of episodes to run
...
#       
    ...
    pool=Pool(processes=os.cpu_count()-1, maxtasksperchild=200)#os.cpu_count())
...
#                   print(best.fitness, best.size(),p.species.get_species_id(best.key),best.key)
               ...
               if p.best_genome is not None:
                   while error_ref:
                       error_count=0
                       try:
                           ref_result=pool.starmap(eval_genomes, zip(repeat(1), repeat(list([0, p.best_genome]),5), repeat(p.config), [1,2,3,4,10], repeat(trafic)))
                           for fitness in ref_result:
                               if fitness==10000:
                                   error_count+=1          
                           best_current_gen=pool.starmap(eval_genomes, zip(repeat(1), repeat(list([0, best]),5), repeat(p.config), [1,2,3,4,10], repeat(trafic)))
...
#   
    ...
    if visualize is not None:
        ...
        visualize.draw_net(config, p.best_genome, True, filename='H:\\MT\\Python\\NEAT und SUMO\\saved models\\'+'best_genome_net'+nowstr+'.svg', node_names=node_names)
        visualize.plot_stats(stats, ylog=False, view=True, filename='H:\\MT\\Python\\NEAT und SUMO\\saved models\\'+'best_genome_stats'+nowstr+'.svg',)
        visualize.plot_species(stats, view=True, filename='H:\\MT\\Python\\NEAT und SUMO\\saved models\\'+'best_genome_species'+nowstr+'.svg')
