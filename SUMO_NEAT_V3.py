...
from math import *
from SUMO_NEAT_Population_V2 import Population
import neat
import visualize
...
#             net = neat.nn.FeedForwardNetwork.create(genome, config)
             ...
             SUMO.a_set[SUMO.step] = 20*SUMO.a_set[SUMO.step]-10#[0,0], state[0,1], state[0,2], state[0,3])
...
#             else:
             ...
             if trafic.vehicle2_exist and trafic.vehicle_2.ID in SUMO.currentvehiclelist:
                 if trafic.vehicle_2.end == False:
                     if simulation.vehicle.getLaneID(trafic.vehicle_2.ID) == 'junction_out_0' and traci.vehicle.getLanePosition(trafic.vehicle_2.ID) > 90:
...
#    print(v_episode)
        ...
        """Calculate Reward ================================================================================"""
        fitness = len(v_episode)*np.sqrt(np.sum(np.square(v_episode)))-np.sum(0.5*np.sign(features.v_allowed-np.asarray(v_episode))+1)-np.mean(np.square(a_episode))-1/sum(np.square(np.asarray(v_episode)*3.6-np.asarray(distance_episode)))-100000*int(SUMO.Collision)
...
#            print('No previous critic model file found. Initialising a new model...')
    ...
    if trafic.training or sample_generation:
        for sim in sim_liste:
            traci.start(['sumo', '-c', 'SUMO_config.sumocfg', '--no-warnings'], label=sim)
    else:
